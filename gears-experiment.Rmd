...
#Plotting Distance
ggplot(data=distance, aes(x=Distance, group=Distance, y=Pcorotation, color=Participant))+
...
occlusion<- reports%>%
  ...
  mutate(Display=as.factor(Display),
         mutate(Percept=as.factor(Percept),
         Percept=fct_recode(Occlusion, "0"="None"),
         fct_level(0, "first level"))%>%
...
distancefit<- brm(PCorotationAdj ~ Distance + (1|Participant), family=Beta(), data=occlusion, 
