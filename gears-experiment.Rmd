...
distance <-
  ...
  dplyr::mutate(NotInteracting= (Distance > "-8.13"))
...
```
```{r}
perceptual_coupling <- fit_prototype(perceptual_coupling_prototype, distance)
independent_model <- fit_prototype(independent_prototype, distance)
physics <- fit_prototype(physics_prototype, distance)
```

```{r comparing the three models ELPD}
models_elpd<- 
...
mutate("ΔELPD"=(sprintf("%.1f±%.2f", elpd_diff, se_diff)))%>%
select(Model, "ΔELPD")

```

```{r comparing the three models weight}
models_weight <-
...
model_comparison <-
  dplyr::left_join(models_elpd, models_weight, by="Model")
```
...
distance_alpha <- compute_full_model_alpha(distance_perceptual_coupling)
distance_beta <- compute_full_model_beta(distance_perceptual_coupling)  

# Compare the two models with term (full) and without it (intercept)
distance_comparison <- compare_models(distance_perceptual_coupling, distance_intercept_model)
...
set.seed(9789931)
occlusion_full_model <- fit_prototype(perceptual_coupling_prototype, occlusion)
occlusion_intercept_model <- fit_prototype(independent_prototype, occlusion)
occlusion_factor_model <- fit_prototype(pairwise_comparison_prototype, occlusion, add_loo = FALSE)
```
