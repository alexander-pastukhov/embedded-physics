...
lmerDistance <- betamix(PCorotationAdj ~  Distance|Participant, data=distance, k=1)
```
...
distance_compare <- loo::loo_compare(list("Distance" = distance_fit$criteria$loo, "Intercept" = distance_null_model$criteria$loo))

distance_weight <- loo::loo_model_weights(list("Distance" = distance_fit$criteria$loo, "Intercept" = distance_null_model$criteria$loo))
```
...
lmerDistanceAsFactor <- betamix(PCorotationAdj ~  DistanceAsFactor|Participant, data=distance, k=1)

distance_comparison_lmer <-
  summary(lmerDistanceAsFactor)@components[[1]]$Comp.1$mean@.Data %>% 
  data.frame() %>%
  rownames_to_column("Factor") %>%
  mutate(Factor = str_replace_all(Factor, "\\(Intercept\\)", "Intercept")) %>%
  mutate(p = ifelse(Pr...z..<0.0001, "<.0001", 
                    as.character(round(Pr...z.., 4)) %>% str_replace("^0*(\\d+)", "") %>% str_c("=", .)
                    )) %>%
  mutate(Label = ifelse(Factor == "Intercept", "", glue("Z={round(z.value, 1)}\np{p}")))


distance_comparison <- 
  distance_comparison_lmer %>%
  left_join(fixef(distance_as_factor_fit) %>%
              data.frame() %>%
              rownames_to_column("Factor") %>%
              mutate(Est = ifelse(Factor == "Intercept", 
                                  "", 
                                  glue("β={round(Estimate, 2)}\nCI={round(Q2.5, 1)}..{round(Q97.5, 1)}"))),
            by="Factor") %>%
  mutate(Label = str_c(Est, Label, sep="\n"))
```
...
distancePlot_main <-
  ...
  geom_quasirandom(data=distance, method="tukeyDense", width=2,
                aes(x=Distance, group=Distance, y=Pcorotation, color=as.factor(Distance))) + 
  ...
  plot_theme + 
  geom_table_npc(data=distance_inset, aes(npcx=x, npcy=y, label= tb)) +
  theme(legend.position = "none", axis.text.x = element_text(color=c("#000000", four_colors))) 
distancePlot_main
...
ggsave("gears-distance.png", width = 18, height=14, units="cm")
ggsave("gears-distance.pdf", width = 18, height=14, units="cm", device=cairo_pdf)
