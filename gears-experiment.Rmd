...
#alternate table with occlusion as factor
occlusion_as_factor <-
  occlusion %>%
  #changing occlusion to factor
...
occlusion_as_factor_fit<- brm(PCorotationAdj ~ Occlusion + (1|Participant),
                    family=Beta(),
                    data=occlusion_as_factor,
                    prior=c(prior(normal(0,10), class="b")))
...
pairwise_difference_occlusion <- bind_rows(
  ...
  tibble(Difference= samples$b_Occlusion65.04,
         Occlusion="65.04")) %>%
  mutate(Occlusion=factor(Occlusion, levels=c("0", "8.13", "16.26", "32.52", "65.04")))
