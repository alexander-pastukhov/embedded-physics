...
ambiguity_summary <-
  ...
  mutate(`β` = round(Estimate, 4), 
         CI = glue("{round(Q2.5, 2)}..{round(Q97.5, 2)}")) %>%
  select(`β`, CI) %>%
  mutate("ΔELPD" = round(ambiguity_compare[2, 1], 1),
...
ambiguityPlot_main <-
  ggplot(data=ambiguity_fit_predictions, 
      aes(x = Display,group=Display, y=Pcorotation)) + 
  geom_line() +
  geom_hline(yintercept = 0.5, color="white", size= 1) +
  geom_quasirandom(data=ambiguity, method="tukeyDense", width=2,
                aes(x=Display, group=Display, y=Pcorotation)) +
  geom_violin(data = ambiguity_fit_predictions, fill="gray64", color="white", draw_quantiles = c(0.5)) +
  scale_x_discrete("Display", labels = c("ambiguous","weak", "medium","medium-strong","strong"))+
 #sec.axis =  sec_axis(name=NULL, labels=ambiguity_comparison$Label)+
  scale_y_continuous(name="Pcorotation") +
  scale_color_manual(values = c("#000000", four_colors)) +
  plot_theme() + 
...
ggsave(distancePlot_main, filename = "gears-ambiguity.pdf", units="cm", width=14, height=8, device=cairo_pdf)
```
