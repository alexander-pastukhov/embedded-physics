...
#distance_interaction<- fit_prototype(interaction_prototype, distance)
...
```{r}
#compare_models(distance_perceptual_coupling, distance_independent_model, distance_physics, distance_interaction)
...
distance_fit_predictions <- 
  predict(distance_fits$`Perceptual Coupling`,
          newdata = data.frame(Term = seq(-10, 70, 1)),
          probs = c(0.055, 0.25, 0.75, 0.945),
          re_formula = NA) %>%
  as_tibble() %>%
...
#occlusion_interaction<- fit_prototype(interaction_prototype, occlusion)
...
```{r comparing models}
#compare_models(occlusion_perceptual_coupling, occlusion_independent_model, occlusion_physics, occlusion_interaction)
...
occlusion_fit_predictions <- 
  predict(occlusion_fits$`Perceptual Coupling`,
          newdata = data.frame(Term = seq(-10, 70, 1)),
          probs = c(0.055, 0.25, 0.75, 0.945),
          re_formula = NA) %>%
  as_tibble() %>%
...
#ambiguity_interaction<- fit_prototype(interaction_prototype, ambiguity)
...
#compare_models(ambiguity_perceptual_coupling, ambiguity_independent_model, ambiguity_physics, ambiguity_interaction)
...
#ambiguity_fit_predictions <- predict(ambiguity_pairwise_comparison,
...
ambiguity_fit_predictions <- 
  predict(ambiguity_fits$`Perceptual Coupling`,
          newdata = data.frame(TermAsFactor =
                              levels(ambiguity$Display)),
                             summary = FALSE,
                            re_formula = NA) %>%
  data.frame() %>%
pivot_longer(everything(), names_to = "Display", values_to = "Pcounterrotate") %>%
...
ambiguity_plot <-
  ...
  scale_x_continuous(name="Display",
                     ...
                     sec.axis =  dup_axis(name = element_blank(),
                                          labels = compute_factor_level_betas(ambiguity_pairwise_comparison)))+
  scale_y_continuous(name="Pcounter-rotation") +
  plot_theme()
ambiguity_plot
```
...
ggsave('figure-02-gears.png', width=14, height=16, units="cm")
ggsave('figure-02-gears.svg', width=14, height=16, units="cm")
```
