...
```{r compute BF}
compute_BF <- function(perceptual_coupling_model, reduced_model){
  BF <- brms::bayes_factor(perceptual_coupling_model, reduced_model, silent = TRUE)
  sprintf('BF=%.2g', BF$bf)
}
```

```{r compute dELPD}
compute_dELPD <- function(perceptual_coupling_model, independent_model){
  dLOO <-   
    loo::loo_compare(list("Term" = perceptual_coupling_model$criteria$loo,
                          "Intercept" = independent_model$criteria$loo)) %>%
    data.frame() %>%
...
compute_full_model_weight <- function(perceptual_coupling_model, intercepts_model){
  full_model_weight <- 
    loo::loo_model_weights(list("Term" = perceptual_coupling_model$criteria$loo,
...
compare_models <- function(perceptual_coupling_model, intercept_model){
  BF <- compute_BF(perceptual_coupling_model, intercept_model)
  dELPD <- compute_dELPD(perceptual_coupling_model, intercept_model)
  Weight <- compute_full_model_weight(perceptual_coupling_model, intercept_model)
...
```{r full model beta}
compute_full_model_beta <- function(perceptual_coupling_model){
  estimate <- 
    fixef(perceptual_coupling_model, probs = c((1-0.89)/2, 1-(1-0.89)/2)) %>%
    data.frame() %>%
    rownames_to_column("Coefficient") %>%
...
```{r full model alpha}
compute_full_model_alpha <- function(perceptual_coupling_model){
  alpha <-
    fixef(perceptual_coupling_model,probs= c((1-0.89)/2, 1-(1-0.89)/2)) %>%
    data.frame()%>%
    rownames_to_column("Coefficient")%>%
...
sprintf('α=%.2f [%0.2f..%0.2f]',inv.logit(alpha$Estimate),
...
reports <- 
  ...
  mutate(NotInteracting= (Distance > "-8.13"))
...
```{r brms models}
full_prototype <- brm(Padj ~ Term + (1|Participant),
                     family=Beta(),
                     data=distance,
                     chains=0, # this means that model is never sampled
                     prior=c(prior(normal(0, 1), class="b")))

intercept_prototype <- brm(Padj ~ (1|Participant),
                           family=Beta(),
                           chains=0, # this means that model is never sampled
                           data=distance)

factor_prototype <- brm(Padj ~ TermAsFactor + (1|Participant),
                        family=Beta(),
                        data=distance,
                        chains=0, # this means that model is never sampled
                        prior=c(prior(normal(0,1), class="b")))
...
set.seed(31937)
disambiguation_factor_model <- fit_prototype(factor_prototype, disambiguation)
disambiguation_intercept_model <- fit_prototype(intercept_prototype, disambiguation)
```
