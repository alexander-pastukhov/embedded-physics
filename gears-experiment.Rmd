...
reports <- 
  ...
  mutate(Percept=as.factor(Percept),
         ...
         DisplayRight=fct_recode(DisplayRight, "strong"="gear24-strong", "medium-strong"="gear24-medium-strong", "medium"="gear24-medium", "weak"="gear24-weak", "ambiguous"= "gear24"),
DisplayRight=fct_relevel(DisplayRight, "ambiguous", "weak", "medium", "medium-strong", "strong"))%>%
  ...
  mutate(Display = ifelse(DisplayLeft=="ambiguous", DisplayRight, DisplayLeft))%>% #turn into factor
...
ggplot(data=distance, aes(x=Distance, group=Distance, y=Pcorotation, color=Participant))+ ## the mistake is caused by mutating distance colum but how do I fix it?
  ...
  geom_beeswarm() + 
  # geom_quasirandom()+
  scale_x_discrete(breaks=c(-10, 0, 10, 20, 30, 40))+
...
```{r}
distancefit2<- brm(PCorotationAdj ~ Distance + (1|Participant), family=Beta(), data=distance)
```
```{r}
plot(distancefit2)
```
