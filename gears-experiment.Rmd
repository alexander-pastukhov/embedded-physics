...
ambiguityPlot_main <-
  ...
  scale_color_manual(values = c("#000000", four_colors)) +
  plot_theme() +
   labs(subtitle=glue("β = {ambiguity_summary$Value[1]}, CI={ambiguity_summary$Value[2]},Bayes Factor= {BF_ambiguity_as_factor_rounded}"))+
...
disambiguation<- reports%>%
  ...
  group_by(Participant, Display)%>% 
  summarize(TotalDuration=sum(Duration),
    CClockwiseDur=sum(Duration[RightGear== "CCW" | LeftGear=="CCW"]),
    PCClockwise=CClockwiseDur/TotalDuration)%>%
  mutate(PCClockwiseAdj=PCClockwise*0.999+0.0005)
```

```{r fitting disambiguation/ambiguity as factor}
set.seed(5516800)
disambiguation_as_factor_fit <- brm(PCClockwiseAdj ~ Display +(1|Participant),
                     family= Beta(),
                     data= disambiguation,
                     refresh=0,
                     cores=future::availableCores(),
...
disambiguation_fit_predictions <- predict(disambiguation_as_factor_fit,
                                     ...
                                     re_formula = NA) %>%
  data.frame() %>%
  pivot_longer(everything(), names_to = "Display", values_to = "PCClockwise") %>%
...
disambiguation_fit_moments <-
  ...
  group_by(Display) %>%
  summarise(meanP = mean(PCClockwise),
            medianP= median(PCClockwise))


```

```{r disambiguation/ambiguity importance via loo}
disambiguation_null_model <- brm(PCClockwiseAdj ~ (1|Participant),
                           family=Beta(),
                           refresh=0,
                           cores=future::availableCores(),
                           save_all_pars = TRUE,
...
disambiguationPlot_main <-
  ggplot(data=disambiguation_fit_predictions, 
      aes(x = as.numeric(Display), group=Display, y=PCClockwise)) + 
  geom_line() +
  geom_hline(yintercept = 0.5, color="white", size= 1) +
  ...
  scale_y_continuous(name="Pcounterclockwise") +
  scale_color_manual(values = c("#000000", four_colors)) +
  plot_theme() +
