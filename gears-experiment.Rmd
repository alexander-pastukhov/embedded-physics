...
```{r disambiguiation sanity check}
disambiguation <-reports%>%
filter(Condition=="Ambiguity")%>% 
  filter(Percept!="unclear")%>%
  group_by(Participant)%>%
  summarize(TotalCorotation=sum(Percept=="Corotation"),
            Disambiguation=sum(DisplayLeft !="ambiguous", DisplayRight != "ambiguous"),
            Pdisambiguation=Disambiguation/TotalCorotation)%>%
  mutate(PdisambiguationAdj= Pdisambiguation*0.999+0.0005)

```

```{r disambiguation duration}
disambiguationDur <-reports%>%
filter(Condition=="Ambiguity")%>% 
  filter(Percept!="unclear")%>%
  group_by(Participant)%>%
  summarize(TotalCorotation=sum(Duration[Percept=="Corotation"]),
            Disambiguation=sum(Duration[DisplayLeft !="ambiguous"| DisplayRight !="ambiguous"]),
            Pdisambiguation=Disambiguation/TotalCorotation)%>%
  mutate(PdisambiguationAdj= Pdisambiguation*0.999+0.0005)

ambiguityPlot_main <-
  ggplot(data=disambiguationDur, 
  ...
  geom_quasirandom(data=ambiguity, method="tukeyDense", width=0.1,
                aes(x=as.numeric(Display), group=Display, y=PdisambiguationAdj)) +
