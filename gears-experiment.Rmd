...
PClockwise_fit_predictions <- predict(PClockwise_as_factor_fit,
                                     newdata = data.frame(PClockwise = levels(disambiguation$PClockwise)),
                                     summary = FALSE,
                                     re_formula = NA) %>%
  data.frame() %>%
  pivot_longer(everything(), names_to = "PClockwise", values_to = "Pclockwise") %>%
  mutate(PClockwise = factor(PClockwise, levels = sort(unique(PClockwise)), labels = levels(disambiguation$PClockwise)))
