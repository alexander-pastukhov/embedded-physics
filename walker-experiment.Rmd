...
```{r brms models}
full_prototype <- brm(Pcongruent ~ Term + (1|Participant),
                     family=Beta(),
                     data=vertical_distance,
                     chains=0, # this means that model is never sampled
                     prior=c(prior(normal(0, 1), class="b")))

intercept_prototype <- brm(Pcongruent ~ (1|Participant),
                           family=Beta(),
                           chains=0, # this means that model is never sampled
                           data=vertical_distance)

factor_prototype <- brm(Pcongruent ~ TermAsFactor + (1|Participant),
                        family=Beta(),
                        data=vertical_distance,
                        chains=0, # this means that model is never sampled
                        prior=c(prior(normal(0,1), class="b")))
...
set.seed(5515605 )
vertical_distance_full_model <- fit_prototype(full_prototype, vertical_distance)
vertical_distance_intercept_model <- fit_prototype(intercept_prototype, vertical_distance)
vertical_distance_factor_model <- fit_prototype(factor_prototype, vertical_distance, add_loo = FALSE)
```

### Full model summary vertical distance

```{r}
# Compute estimates for individual parameters
vertical_distance_alpha <- compute_full_model_alpha(vertical_distance_full_model)
vertical_distance_beta <- compute_full_model_beta(vertical_distance_full_model)  

# Compare the two models with term (full) and without it (intercept)
vertical_distance_comparison <- compare_models(vertical_distance_full_model,vertical_distance_intercept_model)

# summary text
vertical_distance_summary <- stringr::str_c(vertical_distance_alpha, vertical_distance_beta, vertical_distance_comparison, sep=', ')
...
vertical_distance_fit_predictions <- predict(vertical_distance_full_model,
...
vertical_distance_plot <-
  ...
  scale_x_continuous(name = "Distance [%]", breaks = unique(vertical_distance$Distance),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_betas(vertical_distance_factor_model))) +
  scale_y_continuous(name="Pcongruent") +
  plot_theme() +
  labs(subtitle=vertical_distance_summary)

vertical_distance_plot
```

```{r}
print(vertical_distance_plot)
```
...
set.seed(5515605 )
horizontal_distance_full_model <- fit_prototype(full_prototype, horizontal_distance)
horizontal_distance_intercept_model <- fit_prototype(intercept_prototype, horizontal_distance)
horizontal_distance_factor_model <- fit_prototype(factor_prototype, horizontal_distance, add_loo = FALSE)
```

### Full model summary horizontal distance

```{r}
# Compute estimates for individual parameters
horizontal_distance_alpha <- compute_full_model_alpha(horizontal_distance_full_model)
horizontal_distance_beta <- compute_full_model_beta(horizontal_distance_full_model)  

# Compare the two models with term (full) and without it (intercept)
horizontal_distance_comparison <- compare_models(horizontal_distance_full_model,horizontal_distance_intercept_model)

# summary text
horizontal_distance_summary <- stringr::str_c(horizontal_distance_alpha, horizontal_distance_beta, horizontal_distance_comparison, sep=', ')
...
# predict GROUP-LEVEL Pcorotation
horizontal_distance_fit_predictions <- predict(horizontal_distance_full_model,
                                    newdata = data.frame(Term = seq(0, 4.1, 0.1)),
                                    probs = c(0.025, 0.25, 0.75, 0.975),
                                    re_formula = NA) %>%
  data.frame() %>%
...
horizontal_distance_plot <-
  ...
  scale_x_continuous(name = "Horizontal Distance (degrees)", breaks = unique(horizontal_distance$ShiftAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_betas(horizontal_distance_factor_model))) +
  scale_y_continuous(name="Pcongruent") +
  plot_theme() +
  labs(subtitle=horizontal_distance_summary)
...
set.seed(5515605 )
sphere_ambiguity_full_model <- fit_prototype(full_prototype, sphere_ambiguity)
sphere_ambiguity_intercept_model <- fit_prototype(intercept_prototype, sphere_ambiguity)
sphere_ambiguity_factor_model <- fit_prototype(factor_prototype, sphere_ambiguity, add_loo = FALSE)
```

### Full model summary sphere ambiguity

```{r}
# Compute estimates for individual parameters
sphere_ambiguity_alpha <- compute_full_model_alpha(sphere_ambiguity_full_model)
sphere_ambiguity_beta <- compute_full_model_beta(sphere_ambiguity_full_model)  

# Compare the two models with term (full) and without it (intercept)
sphere_ambiguity_comparison <- compare_models(sphere_ambiguity_full_model,sphere_ambiguity_intercept_model)

# summary text
sphere_ambiguity_summary <- stringr::str_c(sphere_ambiguity_alpha, sphere_ambiguity_beta, sphere_ambiguity_comparison, sep=', ')
...
# predict GROUP-LEVEL Pcorotation
sphere_ambiguity_fit_predictions <- predict(sphere_ambiguity_full_model,
                                    newdata = data.frame(Term = seq(0, 4.1, 0.1)),
                                    probs = c(0.025, 0.25, 0.75, 0.975),
                                    re_formula = NA) %>%
  data.frame() %>%
  add_column(Term = seq(0, 4.1, 1)) %>%
...
sphere_ambiguity_plot <-
  ...
  scale_x_continuous(name = "Sphere Disambiguation (degrees)", limits= c(-0.09, 1.09), breaks = unique(sphere_ambiguity$SphereDisambiguationAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_betas(sphere_ambiguity_factor_model))) +
  scale_y_continuous(name="Pcongruent", limits= c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  plot_theme() +
  labs(subtitle=sphere_ambiguity_summary)
...
set.seed(5515605 )
walker_ambiguity_full_model <- fit_prototype(full_prototype, walker_ambiguity)
walker_ambiguity_intercept_model <- fit_prototype(intercept_prototype, walker_ambiguity)
walker_ambiguity_factor_model <- fit_prototype(factor_prototype, walker_ambiguity, add_loo = FALSE)
```

### Full model summary walker ambiguity

```{r}
# Compute estimates for individual parameters
walker_ambiguity_alpha <- compute_full_model_alpha(walker_ambiguity_full_model)
walker_ambiguity_beta <- compute_full_model_beta(walker_ambiguity_full_model)  

# Compare the two models with term (full) and without it (intercept)
walker_ambiguity_comparison <- compare_models(walker_ambiguity_full_model,walker_ambiguity_intercept_model)

# summary text
walker_ambiguity_summary <- stringr::str_c(walker_ambiguity_alpha, walker_ambiguity_beta, walker_ambiguity_comparison, sep=', ')
...
# predict GROUP-LEVEL Pcorotation
walker_ambiguity_fit_predictions <- predict(walker_ambiguity_full_model,
                                    newdata = data.frame(Term = seq(0, 1)),
                                    probs = c(0.025, 0.25, 0.75, 0.975),
                                    re_formula = NA) %>%
  data.frame() %>%
  add_column(Term = seq(0, 1)) %>%
...
walker_ambiguity_plot <-
  ...
  scale_x_continuous(name = "Walker Disambiguation (degrees)",limits= c(-0.09, 1.09),  breaks = unique(walker_ambiguity$WalkerDisambiguationAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_betas(walker_ambiguity_factor_model))) +
  scale_y_continuous(name="Pcongruent", limits= c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  plot_theme() +
  labs(subtitle=walker_ambiguity_summary)
...
```{r}
vertical_distance_plot/horizontal_distance_plot / sphere_ambiguity_plot / walker_ambiguity_plot + 
  plot_annotation(tag_levels= "A")

#ggsave('figure-05-walker.pdf', width=14, height=20, units="cm", device=cairo_pdf)
#ggsave('figure-05-walker.png', width=14, height=20, units="cm")
#ggsave('figure-05-walker.svg', width=14, height=20, units="cm")
```
...
SOR_walker_full_model <- fit_prototype(full_prototype, SOR_walker)
SOR_walker_intercept_model <- fit_prototype(intercept_prototype, SOR_walker)
SOR_walker_factor_model <- fit_prototype(factor_prototype, SOR_walker, add_loo = FALSE)
```

### Full model summary SOR walker

```{r}
# Compute estimates for individual parameters
SOR_walker_alpha <- compute_full_model_alpha(SOR_walker_full_model)
SOR_walker_beta <- compute_full_model_beta(SOR_walker_full_model)  

# Compare the two models with term (full) and without it (intercept)
SOR_walker_comparison <- compare_models(SOR_walker_full_model,SOR_walker_intercept_model)

# summary text
SOR_walker_summary <- stringr::str_c(SOR_walker_alpha, SOR_walker_beta, SOR_walker_comparison, sep=', ')
SOR_walker_summary
```
...
# predict GROUP-LEVEL Pcorotation
SOR_walker_fit_predictions <- predict(SOR_walker_full_model,
...
SOR_walker_plot <-
  ...
  scale_x_continuous(name = "Shift (degrees)", breaks = unique(SOR_walker$ShiftAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_betas(SOR_walker_factor_model))) +
  scale_y_continuous(name="Pcongruent", limits= c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  plot_theme() +
  labs(subtitle=SOR_walker_summary)
...
set.seed(5515605 )
# error here: 'beta' requires response greater than zero
SOR_sphere_full_model <- fit_prototype(full_prototype, SOR_sphere)
SOR_sphere_intercept_model <- fit_prototype(intercept_prototype, SOR_sphere)
SOR_sphere_factor_model <- fit_prototype(factor_prototype, SOR_sphere, add_loo = FALSE)
```

### Full model summary SOR sphere

```{r}
# Compute estimates for individual parameters
SOR_sphere_alpha <- compute_full_model_alpha(SOR_sphere_full_model)
SOR_sphere_beta <- compute_full_model_beta(SOR_sphere_full_model)  

# Compare the two models with term (full) and without it (intercept)
SOR_sphere_comparison <- compare_models(SOR_sphere_full_model,SOR_sphere_intercept_model)

# summary text
SOR_sphere_summary <- stringr::str_c(SOR_sphere_alpha, SOR_sphere_beta, SOR_sphere_comparison, sep=', ')
SOR_sphere_summary
```
...
# predict GROUP-LEVEL Pcorotation
SOR_sphere_fit_predictions <- predict(SOR_sphere_full_model,
                                    newdata = data.frame(Term = seq(0, 4.2)),
                                    probs = c(0.025, 0.25, 0.75, 0.975),
                                    re_formula = NA) %>%
  data.frame() %>%
...
SOR_sphere_plot <-
  ...
  scale_x_continuous(name = "Shift (degrees)", breaks = unique(SOR_sphere$ShiftAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_betas(SOR_sphere_factor_model))) +
  scale_y_continuous(name="Pconsistent", limits= c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  plot_theme() +
  labs(subtitle=SOR_sphere_summary)
