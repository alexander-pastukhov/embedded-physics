...
compare_models <- function(models){
  ...
  models_alpha <-
    purrr::map_df(models, ~compute_model_alpha(.)) %>%
    t() %>%
    data.frame() %>%
    rename(a = 1) %>%
    rownames_to_column("Model")
  ...
  models_beta <-
    purrr::map_df(models, ~compute_model_beta(.)) %>%
    t() %>%
    data.frame() %>%
    rename(b = 1) %>%
    rownames_to_column("Model")
...
```{r factor model pairwise predicted difference}
compute_factor_level_predicted_difference <- function(factor_model, term_levels, CI=0.89){
  
  factor_model <- vertical_pairwise_comparison
  term_levels <- sort(unique(vertical_distance$DistanceAsFactor))
  
  predictions <- 
    predict(factor_model, 
            newdata = data.frame(TermAsFactor = term_levels),
            summary = FALSE,
            re_formula = NA) 
...
vertical_distance_fit_predictions <- predict(vertical_fits$`Perceptual Coupling`,
...
vertical_distance_plot <-
  ...
  scale_x_continuous(name = "Vertical Distance (degrees)", breaks = unique(vertical_distance$Distance),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_predicted_difference(vertical_pairwise_comparison, sort(unique(vertical_distance$DistanceAsFactor))))) +
...
# predict GROUP-LEVEL Pcorotation
horizontal_distance_fit_predictions <- predict(horizontal_fits$`Perceptual Coupling`,
                                    newdata = data.frame(Term = seq(0, 4.1, 0.1)),
                                    probs = c(0.055, 0.25, 0.75, 0.945),
                                    re_formula = NA) %>%
  data.frame() %>%
...
horizontal_distance_plot <-
  ...
  scale_x_continuous(name = "Horizontal Distance (degrees)", breaks = unique(horizontal_distance$ShiftAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_predicted_difference(horizontal_pairwise_comparison, sort(unique(horizontal_distance$ShiftAsFactor))))) +
  scale_y_continuous(name="Pcongruent") +
  plot_theme()
horizontal_distance_plot
```
...
sphere_ambiguity_plot <-
  ...
  scale_x_continuous(name = "Sphere Disambiguation (degrees)", breaks = unique(sphere_ambiguity$SphereDisambiguationAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_predicted_difference(sphere_pairwise_comparison, sort(unique(sphere_ambiguity$SphereDisambiguationAsFactor))))) +
...
# predict GROUP-LEVEL Pcorotation
walker_ambiguity_fit_predictions <- predict(walker_fits$`Perceptual Coupling`,
                                    newdata = data.frame(Term = seq(0, 1.1, 0.1)),
                                    probs = c(0.055, 0.25, 0.75, 0.945),
                                    re_formula = NA) %>%
  data.frame() %>%
...
walker_ambiguity_plot <-
  ...
  scale_x_continuous(name = "Walker Disambiguation (degrees)",limits= c(-0.09, 1.09),  breaks = unique(walker_ambiguity$WalkerDisambiguationAbs),
                     sec.axis =  dup_axis(name=NULL, labels=compute_factor_level_predicted_difference(walker_pairwise_comprarison, sort(unique(walker_ambiguity$WalkerDisambiguationAsFactor))))) +
  scale_y_continuous(name="Pcongruent", limits= c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  plot_theme()
walker_ambiguity_plot
```
