...
SOR_walker<- 
  reports %>%
  filter(SessionTag=="Addendum", Condition %in% c("Walker"))%>%
...
SOR_sphere<- 
  reports %>%
  filter(SessionTag=="Addendum", Condition %in% c("Sphere"))%>%
  filter(Participant != "LLL2000WRNO")%>%
  ...
  dplyr::summarize(TotalDuration = sum(Duration),
            # if value in the brackets is true it'll be considered if not it will not
            CongruentDur = sum(Duration[Percept=="left"]),               ## EXHNAGE IS LEFT OR RIGHT CONGRUENT?!!!!!!!!!!!!!!!!!!!!!
            Pcongruent = CongruentDur / TotalDuration) %>%
  dplyr::mutate(PcongruentAdj = Pcongruent*0.999+0.0005)
...
set.seed(83661)
SOR_sphere_fit <- brm(PcongruentAdj ~ ShiftAbs + (1|Participant),
                    family=Beta(),
                    data=SOR_sphere,
                    cores=future::availableCores(),
                    save_all_pars = TRUE,
...
set.seed(81810)
SOR_sphere_as_factor_fit <- brm(PcongruentAdj ~ ShiftAsFactor + (1|Participant),
                              family=Beta(), 
                              cores = future::availableCores(),
                              data=SOR_sphere,
                              prior=c(prior(normal(0,10), class="b")))
...
SOR_sphere_plot_main <-
  ...
  geom_quasirandom(data=SOR_sphere, method="tukeyDense", width=0.2,
                aes(x=ShiftAbs, group=ShiftAbs, y=Pcongruent)) + 
  theme(legend.position = "none") + 
  scale_x_continuous(name = "Shift", breaks = unique(SOR_sphere$ShiftAbs),
